defmodule PentoWeb.SurveyResultsLiveTest do
  ...
  describe "Socket state" do
    ...
    test "no ratings exist", %{socket: socket, product: product} do
      socket =
        socket
      ...
      assert socket.assigns.products_with_average_ratings == [{product.name, 0}]
    end
    ...
    test "ratings exist", %{socket: socket, product: product, user: user} do
      ...
      assert socket.assigns.products_with_average_ratings == [{product.name, 2.0}]
    end
    ...
    test "ratings are filtered by age group", %{
      ...
      |> SurveyResultsLive.assign_gender_filter()
      |> SurveyResultsLive.assign_products_with_average_ratings()
      |> assert_keys(:products_with_average_ratings, [{product.name, 2.0}])
    end
